...
""", height=130, key="voice_input") # keyを追加して再実行時の動作を安定させる

final_prompt = prompt or voice_prompt

if final_prompt:
    # ★★★ 変更点：入力が文字列の場合のみ処理を実行 ★★★
    if isinstance(final_prompt, str):
        # 同じメッセージが重複して処理されるのを防ぐ
        if "last_prompt" not in st.session_state or st.session_state.last_prompt != final_prompt:
